...
@pytest.mark.asyncio
async def test_normal_message_not_blocked():
    from src.openclaw.chat.client import ChatClient, _INJECTION_PATTERNS
    client = ChatClient(make_cfg())
    # "help" should not match injection pattern
...
async def test_rate_limit_enforced():
    ...
    client._api_key = "fake"
    # Fill the rate limit window manually
    import time
...
async def test_session_created_lazily():
    from src.openclaw.chat.client import ChatClient
    client = ChatClient(make_cfg())
    assert client._session is None  # not created until first API call
    await client.close()            # should not raise even if session is None
